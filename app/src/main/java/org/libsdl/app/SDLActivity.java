...
public static boolean downloadFileFromUrl(String urlString, String savePath) {
    ...
    try {
        ...
        urlConnection.setConnectTimeout(5000); // 5秒連線超時
        urlConnection.setReadTimeout(10000);   // 10秒讀取超時
        urlConnection.setDoInput(true);
        urlConnection.connect();

        int responseCode = urlConnection.getResponseCode();
        if (responseCode != HttpURLConnection.HTTP_OK) {
            Log.e("SDL", "Server returned HTTP " + responseCode + " " + urlConnection.getResponseMessage());
            return false;
        }
        ...
        Log.d("SDL", "Download success: " + savePath);
        return true;

    } catch (Exception e) {
        Log.e("SDL", "Download failed: " + e.toString());
        // 如果下載失敗，刪除可能損壞的檔案
        try { new java.io.File(savePath).delete(); } catch (Exception ignored) {}
        return false;
    } finally {
        try {
...
*/
	...
	// ---------------------------------------------------------
    	// ★ 強制全螢幕與沉浸模式 (Java 代碼版，比 XML 更可靠)
    	// ---------------------------------------------------------
    	if (Build.VERSION.SDK_INT >= 28) {
        	// 允許視窗延伸到瀏海區域 (這行是解決 2700 vs 2856 的關鍵)
	...
	int keyCode = event.getKeyCode();
        ...
        if (keyCode == KeyEvent.KEYCODE_TAB) {
            ...
            // ★★★ 最關鍵的一行：Return true ★★★
            // 告訴 Android：「這個按鍵我處理完了，你閉嘴，不要彈鍵盤，不要切換焦點」
            return true; 
        }        
	if (SDLActivity.mBrokenLibraries) {
           return false;
