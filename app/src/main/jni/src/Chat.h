...
//DisplayChatBackCustom  Ÿ NEW_CHAT_IMG,0 +3n   7 
...
const int CHAT_DELAY_TYPE_MERCHANT_PC	= 500;// 5  
...
const int	MAX_CHAT_COUNT					= 10;//ä   μ
const int	MAX_DISPLAY_CHAT_WIDTH_BY_PIXEL	= 410;
const int	MAX_CHAT_DISPLAY_TIME			= 30000;//ȭ鿡   ִ ð.
const int	MAX_CHAT_STAY_TIME				= 60000;//ȭ鿡        ð,

const int	MAX_NORMAL_COUNT		=	200;
const int	MAX_WHISPER_COUNT		=	200;	
const int	MAX_GUILD_COUNT			=	200;
const int	MAX_GUILDBBS_COUNT		=	200;
const int	MAX_PARTY_COUNT			=	200;
const int	MAX_NATIONWAR_COUNT		=	200;
const int	MAX_LOCALWAR_COUNT		=	200;
const int	MAX_COLOSUSS_COUNT		=	200;
const int	MAX_YES_NO_COUNT		=	1;
const int	MAX_NOTICE_COUNT		=	200;
const int	MAX_NATIONCHAT_COUNT	=	200;
enum ChatGroups
{
	NORMAL_CHAT_GROUP				=0,
	GUILD_CHAT_GROUP				=1,
...
class CChatData; // Forward declaration for the function pointer type

// Define the callback function pointer type for menu actions
using MenuActionCallback = void(*)(CChatData*);
...
public:
	CChatData()
	{
		...
		m_szChatTime[0] = '\0';
		bIsHaveHeader = false;
		m_nameRect = {0, 0, 0, 0}; // Initialize the name rectangle
		m_bHasItemLink = false;
		m_iItemID = 0;
	}
	~CChatData()
	{
...
public:
	int			m_iIndex;
	int			m_iIndex2;
	SDL_Rect    m_nameRect; // Bounding box for the username on screen
	bool		m_bHasItemLink;
	int			m_iItemID;
...
private:
	...
	DWORD		m_dwWriteTime;
	DWORD		m_dwMaxDisplayTime;//    ִ ð.//̽ð Ѿٰ  
...
public:
	...
	void SetNameRect(const SDL_Rect& rect) { m_nameRect = rect; } // <--- 加在這裡
	void SetData(const char* szValue)
	{
		const int iDataLen = strlen(szValue);
		if( 0 >= iDataLen ) {return;}//Ÿ ̰ ̻ϸ Է Ǹ ȵȴ
	...
	{
		if( NULL == szValue || 0== szValue[0]){m_szName[0] = '\0'; return;}//
	...
	{
		if( NULL == szValue || 0== szValue[0]){m_szHeader[0] = NULL; return;}//
		...
		{
			{m_szHeader[0] = '\0'; return;}//   (
		}
	}
	void SetChatType(const int iValue,const int iIndex1,const int iIndex2)	//ä Ÿ     ð   ð      Ѵ
	{
		m_iChatType=iValue;
		m_dwWriteTime=SDL_GetTicks();
		m_dwMaxDisplayTime = m_dwWriteTime+MAX_CHAT_DISPLAY_TIME;//20ʰ ÷   
		m_iIndex = iIndex1;
		m_iIndex2= iIndex2;
...
*/
	...
	void SetAcceptWhiper(const char* szName)
	{
		...
		{
			if(itor.compare(szName) == 0)
			{
				vRefuseList.erase(std::remove(vRefuseList.begin(), vRefuseList.end(), itor), vRefuseList.end());
...
*/
	...
	int	 IsRefuseAllWhisper(){return m_iRefuseAllWhisper;}//true̸ Ӹ źλ
...
public:
	void AddString(const char* szName, const char* szChatData, const int iChatType);
	void ChatLog(chatitor itor);
	//ä Ÿ ߰
	void DeleteCheck();//  üũ 
	void DisplayCheck();//  üũ 
	void DisplayData(const int iX, const int iY,const int iOption = 0);//ȭ鿡 Ѹ//
	...
	int DrawChatData(CChatData* pData, const int iY, const int iX, const int iLineGab);//  κ Դϴ.//   մϴ.
...
public:
    ...
    void OpenContextMenu(CChatData* pData, int x, int y);
...
extern bool IsAbleChatTargetName(const int iType);//ĭ ùٸ ĭΰ(  ȮѰ)
...
extern Spr *chatimage[NEW_CHAT_SPR_MAX];
