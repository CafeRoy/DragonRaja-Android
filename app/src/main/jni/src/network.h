...
const int _MAX_CLIENT_QUEUE_OUT_BUFFER = MAX_CLIENT_QUEUE_OUT_BUFFER;  // 定義緩衝區最大值
...
{
	// 建議的建構函式
     connection():socket(nullptr), socketSet (nullptr),
	 state(0),receive(0),connect_time(0),send_bytes(0),receive_bytes(0),
	 packet_pnt(0),packet_size(0),packet_status(0),Port(0){
		// 預先為緩衝區分配記憶體以提高效率
        sendBuffer.reserve(_MAX_CLIENT_QUEUE_OUT_BUFFER);
		inbuf.clear();
        outbuf.clear();
	...
	// 將 *inbuf 和 *outbuf 替換為 std::list
    std::list<t_packet> inbuf;
    std::list<t_packet> outbuf;
    std::vector<char> sendBuffer;  // 持久化的發送緩衝區
...
extern "C"
{
	...
	extern int		ConnectServer( t_connection *c, std::string lpIP, int nPort );
	extern int		ConnectLogin( t_connection *c, std::string ID, std::string PW ,int LineNum );

	extern void		CheckBytesRoutine(void);

	extern void		SendChatData( std::string ,const int iChatType = CHATTYPE_NORMAL);//020701 lsw
	extern void		RecvChatData( t_server_chat_data *p );
	extern void		RecvNPCChatData(t_server_chat_data* p);
	...
	extern int		HandleReading(t_connection* c);
	...
	extern int ConnectProxyServer(t_connection* c);
#ifdef __cplusplus
}
#endif
