...
#define SharpRandom()      srand( (unsigned __int32)SDL_GetTicks() )

// [Android Fix] 安全讀寫，防止 SIGBUS (對齊) 和 SIGSEGV (越界)
inline __int32 SafeReadInt32(const void* ptr) {
    __int32 val;
    memcpy(&val, ptr, sizeof(__int32));
    return val;
}
inline void SafeWriteInt32(void* ptr, __int32 val) {
    memcpy(ptr, &val, sizeof(__int32));
}
...
CHSEL_STREAM::CHSEL_STREAM(void)
{
    ...
    iHSELType  = 0;
    // 安全初始化
    lpDesEncryptType = 0; lpDesDecryptType = 0;
    lpDesLeftEncrypt = 0; lpDesRightEncrypt = 0; lpDesMiddleEncrypt = 0;
    lpDesLeftDecrypt = 0; lpDesRightDecrypt = 0; lpDesMiddleDecrypt = 0;
    lpSwapEncrypt = 0; lpSwapDecrypt = 0;
}

CHSEL_STREAM::~CHSEL_STREAM(void) {}

__int32 CHSEL_STREAM::GetVersion(void) { return HSEL_VERSION; }
__int32 CHSEL_STREAM::GetHSELType(void) { return iHSELType; }
...
__int32 CHSEL_STREAM::Initial(HSEL_INITIAL hselinit)
{
    ...
    if(hselinit.iEncryptType == HSEL_ENCRYPTTYPE_RAND ) {
        switch(rand()%4) {
            case 0: hselinit.iEncryptType = HSEL_ENCRYPTTYPE_1; break;
            case 1: hselinit.iEncryptType = HSEL_ENCRYPTTYPE_2; break;
            case 2: hselinit.iEncryptType = HSEL_ENCRYPTTYPE_3; break;
            default: hselinit.iEncryptType = HSEL_ENCRYPTTYPE_4; break;
        }
    }
    ...
    switch(hselinit.iCustomize & 0xF000) {
        case HSEL_KEY_TYPE_CUSTOMIZE: SetKeyCustom(hselinit.Keys); break;
        case HSEL_KEY_TYPE_DEFAULT: GenerateKeys(Init.Keys); break;
        default: return 0;
    }
...
void CHSEL_STREAM::SetNextKey(void) {
    Init.Keys.iLeftKey = GetNextKey(Init.Keys.iLeftKey, Init.Keys.iLeftMultiGab, Init.Keys.iLeftPlusGab);
    Init.Keys.iRightKey = GetNextKey(Init.Keys.iRightKey, Init.Keys.iRightMultiGab, Init.Keys.iRightPlusGab);
    Init.Keys.iMiddleKey = GetNextKey(Init.Keys.iMiddleKey, Init.Keys.iMiddleMultiGab, Init.Keys.iMiddlePlusGab);
    Init.Keys.iTotalKey = GetNextKey(Init.Keys.iTotalKey, Init.Keys.iTotalMultiGab, Init.Keys.iTotalPlusGab);
}
...
void CHSEL_STREAM::DESSingleEncode(char *lpStream,const __int32 iStreamSize) { (this->*lpDesLeftEncrypt)(lpStream,iStreamSize); }
void CHSEL_STREAM::DESSingleDecode(char *lpStream,const __int32 iStreamSize) { (this->*lpDesLeftDecrypt)(lpStream,iStreamSize); }
...
// [Android Fix] Signed CRC
void CHSEL_STREAM::GetCRC(char *lpStream,const __int32 iStreamSize) {
    iCRCValue  =  0;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
    ...
    signed char* pSignedStream = (signed char*)lpStream;
    while(iRemainCount) {
        iCRCValue  ^= pSignedStream[iPos++];
        iRemainCount--;
    }
}

__int32 CHSEL_STREAM::GetCRCConvertInt(void){ return iCRCValue; }
...
void CHSEL_STREAM::SwapEncrypt(char *lpStream,const __int32 iStreamSize) {
    iBlockCount    = (iStreamSize>>2);
    if(LIMIT_SWAP_BLOCK_COUNT > iBlockCount)   {return;}
    iBlockCount--;
...
void CHSEL_STREAM::DESLeftEncode_Type_1(char *lpStream,const __int32 iStreamSize) {
    iTempLeftKey = Init.Keys.iLeftKey;
    iBlockCount = (iStreamSize>>2);
    iRemainCount = (iStreamSize&3);
    iPos = (iBlockCount<<2);
    __int32 *lpBlock = (__int32*)lpStream;
...
void CHSEL_STREAM::DESLeftDecode_Type_1(char *lpStream,const __int32 iStreamSize) {
    iTempLeftKey = Init.Keys.iLeftKey;
    iBlockCount = (iStreamSize>>2);
    iRemainCount = (iStreamSize&3);
    iPos = (iBlockCount<<2);
    __int32 *lpBlock = (__int32*)lpStream;
    signed char *lpChar = ((signed char *)&Init.Keys.iLeftKey);
    signed char *lpStreamSigned = (signed char*)lpStream;
    ...
    if(iBlockCount) { (*lpBlock) ^= (Init.Keys.iLeftKey); }
    while(iRemainCount) {
        lpStreamSigned[iPos++] ^= (*(lpChar+iRemainCount));
        iRemainCount--;
    }
}

// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightEncode_Type_1(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
    char *lpBlockPtr = (lpStream+iStreamSize-4);
    char *lpChar = ((char *)&Init.Keys.iRightKey);
    while(iBlockCount) {
        __int32 val = SafeReadInt32(lpBlockPtr);
        val ^= iTempRightKey;
        SafeWriteInt32(lpBlockPtr, val);
        iTempRightKey = SafeReadInt32(lpBlockPtr);
        lpBlockPtr -= 4;
        iBlockCount--;
    }
    while(iRemainCount) {
...
// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightDecode_Type_1(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);

    char *lpBlockPtr = (lpStream + iRemainCount);
    signed char *lpChar = ((signed char *)&Init.Keys.iRightKey);
    signed char *lpStreamSigned = (signed char*)lpStream;

    iTempRightKey = SafeReadInt32(lpBlockPtr + 4);
    ...
    if(iBlockCount) {
        __int32 val = SafeReadInt32(lpBlockPtr);
        val ^= Init.Keys.iRightKey;
        SafeWriteInt32(lpBlockPtr, val);
    }
    while(iRemainCount) {
        --iRemainCount;
        lpStreamSigned[iRemainCount] ^= (*(lpChar+iRemainCount));
    }
}

void CHSEL_STREAM::DESMiddleEncode_Type_1(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
    __int32 *lpBlock = (__int32*)(lpStream);
...
void CHSEL_STREAM::DESMiddleDecode_Type_1(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
    __int32 *lpBlock = (__int32*)(lpStream);
    signed char *lpChar = ((signed char*)&Init.Keys.iMiddleKey);
    signed char *lpStreamSigned = (signed char*)lpStream;
    while(iBlockCount) {
        (*(lpBlock++)) ^= iTempMiddleKey;
        iBlockCount--;
...
void CHSEL_STREAM::DESLeftDecode_Type_2(char *lpStream,const __int32 iStreamSize) {
    iTempLeftKey = Init.Keys.iLeftKey;
    iBlockCount = (iStreamSize>>2);
    iRemainCount = (iStreamSize&3);
    iPos = (iBlockCount<<2);
...
// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightEncode_Type_2(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
    char *lpBlockPtr = (lpStream+iPos+iRemainCount);
    char *lpEndPtr = (lpStream+iStreamSize-4);
    char *lpChar = ((char *)&Init.Keys.iRightKey);
    lpBlockPtr = lpEndPtr;
    while(iBlockCount) {
        __int32 val = SafeReadInt32(lpBlockPtr);
...
// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightDecode_Type_2(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESMiddleEncode_Type_2(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESMiddleDecode_Type_2(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESLeftDecode_Type_3(char *lpStream,const __int32 iStreamSize) {
    iTempLeftKey = Init.Keys.iLeftKey;
    iBlockCount = (iStreamSize>>2);
    iRemainCount = (iStreamSize&3);
    iPos = (iBlockCount<<2);
...
// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightEncode_Type_3(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightDecode_Type_3(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESMiddleEncode_Type_3(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESMiddleDecode_Type_3(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESLeftDecode_Type_4(char *lpStream,const __int32 iStreamSize) {
    iTempLeftKey = Init.Keys.iLeftKey;
    iBlockCount = (iStreamSize>>2);
    iRemainCount = (iStreamSize&3);
    iPos = (iBlockCount<<2);
...
// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightEncode_Type_4(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
// ★★★ [Fix Crash] Memcpy + Bounds Check ★★★
void CHSEL_STREAM::DESRightDecode_Type_4(char *lpStream,const __int32 iStreamSize) {
    iTempRightKey = Init.Keys.iRightKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESMiddleEncode_Type_4(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
...
void CHSEL_STREAM::DESMiddleDecode_Type_4(char *lpStream,const __int32 iStreamSize) {
    iTempMiddleKey = Init.Keys.iMiddleKey;
    iBlockCount = iStreamSize>>2;
    iRemainCount = iStreamSize&3;
    iPos = (iBlockCount<<2);
