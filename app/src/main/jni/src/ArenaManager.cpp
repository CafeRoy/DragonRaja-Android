...
bool CArenaManager::CheckInTeamTimePeroid(DWORD now)
{
	if (m_InTeamTime <= 0) return false;

	//5~7 = 怪物競技場
	//if (m_nSelectGame >= 5 && m_nSelectGame <= 7)
	if(m_nSelectGame > 0)
	{
		if (now - m_InTeamTime >= TIME_ARENA_IN_TEAM_TIME) return true;
	}
	return false;
}
