...
bool CLottoMgr::WinnerMenuTextDisplay(int nMenuNum)
{
	if(nMenuNum == MN_WINNER_MENU && SMenu[MN_WINNER_MENU].bActive )
	{
		...
		else
		{
			...
			Hprint2(x,y,g_DestBackBuf,"%s", lan->OutputMessage(4,98));//���� ��÷�� ���� �ʾҴ�.
		}
		y+= 30;//16;
		Hcolor( FONT_COLOR_NAME ); //<���� ��ȣ> ���.
		Hprint2(x,y,g_DestBackBuf,"%s", lan->OutputMessage(4,97));
		
		Hcolor( FONT_COLOR_NUMBER );
		Hprint2(x+130,y+170,g_DestBackBuf,"( %d / %d )",this->m_nCrruntPage,this->m_nTotalPage);//���������.
		y+= 28;		
		for(int i = 0;i < 5;i++)
		{
...
#ifndef HONGKONG_LOCALIZING_ //<soto-HK030929 ���� ������ ��ġ ����.
	::QueuePacket(&p,1);
#endif//>soto-HK030929
...
{
//	if(pOpenMsg->nViewCount == 0)return;//ǥ���Ұ� ����.
...
void CLottoMgr::RecvBuyLotto(t_BUY_LOTTO *pBuyLotto)
{
	...
	else //���.
	{
		...
		pMsg = "Point";
#endif
		::AddCurrentStatusMessage(0,255,255,lan->OutputMessage(4,95),pBuyLotto->m_nLottoPay,pMsg);//ũ���� �Һ�ȴ�.
		++m_nTotalLottoCount;
		this->m_nTotalPage = this->CalPageCount(m_nTotalLottoCount);
	}	
}
...
{
	if(pCheckWinner->nWinItemCount > 0)//������ ����.
	{
		for(int i = 0;i < 5;++i)
		{
			//kr_lan���� �Ұ�."%���� ��÷�������� ����� ���࿡ ���� �Ǿ����ϴ�."
	...
	else if(pCheckWinner->nWinItemCount == 0)//��.
	{
		::AddCurrentStatusMessage(0,255,255,lan->OutputMessage(4,87));
	}
	else if(pCheckWinner->nWinItemCount == -1)//���� ����.
	{
		::AddCurrentStatusMessage(0,255,255,lan->OutputMessage(4,86));//::AddCurrentStatusMessage(0,255,255,lan->OutputMessage(4,86),pCheckWinner->anWinNumbers[0]);//kr_lan���� �Ұ�."����� ���࿡ %���� ������� �ʿ��մϴ�."
	}
	else if(pCheckWinner->nWinItemCount == -3)//��÷ ���� �ʾҴ�.
	{
...
//#endif
	...
	{	//< CSD-TW-030625 //<soto-HK030929 ���������� ��ġ ����
		t_packet p;
		p.h.header.type = CMD_OPEN_LOTTOMENU;
		p.h.header.size = 0;
		::QueuePacket(&p,1); //soto-030611 ��޼���
