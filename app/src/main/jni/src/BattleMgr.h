...
public:
	...
	void EffectForNpc(MAGICLIST* lpML, LPCHARACTER attacked_Char, USEEFFECTLIST* lpEL);
	...
	void SendMagicBladeExecute(BYTE nMagic, LPCHARACTER pTarget);
	...
	void SendMagicExecute(BYTE nMagic, LPCHARACTER pTarget, int nX, int nY);
	
	// CSD-TW-030606
	void SendMagicResult(WORD wdEffect, LPCHARACTER pCaster, WORD idTarget, int nX, int nY); // CSD-031007
	
	void SendCombatClear();
	void SendChangeTreasuremapXY(LPCHARACTER pMaster, BYTE nPara, BYTE nX, BYTE nY, BYTE nPoint);
	// CSD-021128
	...
	bool IsClassEnableMagic(LPCHARACTER ch, int nNo) const;
	bool IsCombat(int nNo) const;
	bool IsSendCMD_ATTACK(int nNo) const;
public:
...
private:
	...
	bool          m_bDisplay;
	mutable BYTE  m_nDamageType;	
	mutable DWORD  cmd_attack;
};
//
///////////////////////////////////////////////////////////////////////////////
