...
{	
	...
	t_packet p;
	p.h.header.type = g_block_attack ? CMD_BLOCK_ATTACK_START : CMD_BLOCK_ATTACK_END;
...
void SendBlockMagic()
{
	...
	p.h.header.type = g_block_magic ? CMD_BLOCK_MAGIC_START : CMD_BLOCK_MAGIC_END;
...
void SendBlockAntiTeam(void)//reece
{
	...
	t_packet p;
	p.h.header.type = g_block_anti_team ? CMD_BLOCK_ANTI_TEAM_START : CMD_BLOCK_ANTI_TEAM_END;
	p.h.header.size = 0;
...
void SendBlockSkill()//020821 lsw
{
#ifdef _DEBUG
	/*char temp[MAX_PATH];
	if( !g_block_move )
	...
	t_packet p; 
	p.h.header.type = g_block_skill ? CMD_BLOCK_SKILL_START : CMD_BLOCK_SKILL_END;
	p.h.header.size = 0;
	QueuePacket( &p, 1 );
#endif
}
void SendBlockMove()
{
	...
	t_packet p;
	p.h.header.type = g_block_move ? CMD_BLOCK_MOVE_START : CMD_BLOCK_MOVE_END;
...
void SendBlockGiveLife( void )
{
	...
	t_packet p;
	p.h.header.type = g_block_GiveLife ? CMD_BLOCK_GIVE_LIFE_START : CMD_BLOCK_GIVE_LIFE_END;
	p.h.header.size = 0;
...
void ViewCounselerStatus( void )
{
	...
	{	
		...
		::HprintBold(10, 130 + 60, (g_block_anti_team) ? RGB(255, 0, 100) : RGB(0, 255, 100), 0, "block_team=%d", g_block_anti_team);
	}
	...
	if( g_give_life		  ){ HprintBold( 680,25, RGB( 0, 255, 100 ), 0, lan->OutputMessage(7,19) ); }//010215 lsw
	if( g_absolute_move )  { HprintBold( 680,25, RGB( 0, 255, 100 ), 0, lan->OutputMessage(7,20) ); }//010215 lsw
	if( g_absolute_move_dk){ HprintBold( 680,25, RGB( 0, 255, 100 ), 0, lan->OutputMessage(7,21) ); }//010215 lsw
	if( AbsoluteMoveFlag)  { HprintBold( 680,25, RGB( 0, 255, 100 ), 0, lan->OutputMessage(7,22) ); }//010215 lsw
	if( AbsoluteMoveDKFlag){ HprintBold( 680,25, RGB( 0, 255, 100 ), 0, lan->OutputMessage(7,23) ); }//010215 lsw
#else
	if(g_block_attack)::HprintBold(10, 130 + 0, (g_block_attack) ? RGB(255, 0, 100) : RGB(0, 255, 100), 0, lan->OutputMessage(7, 11));
	if (g_block_magic)::HprintBold(10, 130 + 15, (g_block_magic) ? RGB(255, 0, 100) : RGB(0, 255, 100), 0, lan->OutputMessage(7, 13));
	if (g_block_move)::HprintBold(10, 130 + 30, (g_block_move) ? RGB(255, 0, 100) : RGB(0, 255, 100), 0, lan->OutputMessage(7, 15));
	//Shift 指令
	if (Hero && Hero->DontMoveAttackFlag) {
		HprintBold(10, 190, (Hero->DontMoveAttackFlag) ?
			RGB(255, 0, 100) : RGB(0, 255, 100), 0, lan->OutputMessage(0, (Hero->DontMoveAttackFlag) ? 32 : 33));
	}

	if (Hero && Hero->guild_party) {
		HprintBold(10, 205, (Hero->guild_party) ?
			RGB(255, 0, 100) : RGB(0, 255, 100), 0, lan->OutputMessage(0, (Hero->guild_party) ? 21 : 22));
	}

	if (Hero && Hero->nation_party) {
		HprintBold(10, 220, (Hero->nation_party) ?
			RGB(255, 0, 100) : RGB(0, 255, 100), 0, lan->OutputMessage(0, (Hero->nation_party) ? 23 : 24));
	}

	//不會增加經驗值指令
	if (Hero && Hero->addExp) {
		HprintBold(10, 235, (Hero->addExp) ?
			RGB(255, 0, 100) : RGB(0, 255, 100), 0, lan->OutputMessage(0, (Hero->addExp) ? 30 : 29));
	}
#endif
}
...
SDL_Color ViewTipsColor[TIPS_LINE]; // 010215 lsw
RichTextLine Tips[50][ TIPS_LINE];
int nTipsLineBlocks = 0;
int ViewTipsLoad(int n) {
    ...
    // 限制最大區塊數
    nTipsLineBlocks = (n > 49) ? 49 : n;

    // 如果該區塊的第一行已經有東西，代表載入過了，直接返回成功
    if (!Tips[nTipsLineBlocks][0].empty()) return 1;

    if (Hero == NULL) return 0;

    // 2. [Android] 路徑修正：移除開頭的 ./，並使用 snprintf 防止溢位
    // 注意：這裡假設 Hero->name_status.nation 是合法的整數
    switch( SCharacterData.nCharacterData[CLASS] )
    {
        case WARRIOR:  snprintf(f, FILENAME_MAX, "data/warrior_tip%d.txt", Hero->name_status.nation); break;
        case THIEF:    snprintf(f, FILENAME_MAX, "data/thief_tip%d.txt",   Hero->name_status.nation); break;
        case ARCHER:   snprintf(f, FILENAME_MAX, "data/archer_tip%d.txt",  Hero->name_status.nation); break;
        case WIZARD:   snprintf(f, FILENAME_MAX, "data/wizard_tip%d.txt",  Hero->name_status.nation); break;
        case PRIEST:   snprintf(f, FILENAME_MAX, "data/priest_tip%d.txt",  Hero->name_status.nation); break;
        default:       return 0;
    }

    // 3. 讀取檔案 (這裡假設 ReadTipsBlock 內部已經改用 SDL_RWFromFile 了)
    std::vector<RichTextLine> tips = ReadTipsBlock(f, n);

    if(tips.empty()) {
        SDL_Log("ViewTipsLoad: 讀取不到資料或檔案為空: %s", f);
        return 0;
    }
    ...
    // 外層迴圈：遍歷每一「行」 (RichTextLine)
    for (const auto& richLine : tips)
    {
        if (line_index >= TIPS_LINE) break; // 防止陣列越界

        // A. 儲存整行的結構 (包含所有顏色片段)
        Tips[nTipsLineBlocks][line_index] = richLine;

        // B. 處理舊式顏色陣列 (ViewTipsColor)
        // 如果 ViewTipsColor 是一個簡單的 RGB 陣列，它只能存「該行的一種顏色」
        // 我們通常取該行的「第一個片段」的顏色作為代表
        if (!richLine.empty()) {
            // 取第一個片段 (segment) 的顏色
            const auto& firstSegment = richLine[0];
            ViewTipsColor[line_index].r = firstSegment.color.r;
            ViewTipsColor[line_index].g = firstSegment.color.g;
            ViewTipsColor[line_index].b = firstSegment.color.b;
        } else {
            // 如果這行是空的，給個預設白色
            ViewTipsColor[line_index].r = 255;
            ViewTipsColor[line_index].g = 255;
            ViewTipsColor[line_index].b = 255;
        }
...
{			
	...
	Box( Mapx+1         , Mapy+1 + TIPS_Y-4,  SCREEN_WIDTH +Mapx-1, TIPS_YL+Y+Mapy-1, RGB16( 150, 150,150 ));
	Box( Mapx           ,   Mapy + TIPS_Y-4,  SCREEN_WIDTH +Mapx,   TIPS_YL+Y+Mapy,   RGB16( 100, 100,100 ));
...
{	
	...
	else
	{
		...
		HprintBold( 10,5, RGB( 0xff, 0xff, 0xff),  RGB( 0x7f, 0x7f, 0x7f), "Tip ����" );*/
	}	
}		
...
{	
	...
	{
		HprintBoldcolor( X, Y, Tips[nTipsLineBlocks][i]);
		Y += 16;
	}
...
{	
	...
	const int iLGab = CHAT_BG_HEIGHT;//�� ����
	...
	TxtOut.RcChatHeaderOut(iX, iY-iLGab*(iLCt+1), iChatWidth,iGab,CHAT_COLOR_TYPE_DEFAULT,lan->OutputMessage(7,49));//iLCt+1 �� ������ �̸� ����
}		
